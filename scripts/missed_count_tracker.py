...
def handle_missed_detection(tracks, centers, flows, next_track_id):
    ...
    # No step 4: pending_deactivation removed; misses handled globally per frame
...
def track_bees_stepwise(video_path, flow_folder_fwd, flow_folder_bwd, yolo_json_path):
    ...
    while frame_idx < total_frames:
        ...
        if diff == 0:
            ...
            message = "After second missed detection pass."
            visualize_frame(frame, all_detections, frame_idx, tracks, centers, assigned_tracks, num_curr, prev_num_dets, message)
            ...
            message = "After third missed detection pass."
            visualize_frame(frame, all_detections, frame_idx, tracks, centers, assigned_tracks, num_curr, prev_num_dets, message)
        ...
        for tid, tr in list(tracks.items()):
            if tr.active:
                ...
                if tr.miss_count >= 3:
                    tr.active = False
                    message = f"Deactivated track {tr.tid} (miss_count: {tr.miss_count})"
                    visualize_frame(frame, all_detections, frame_idx, tracks, centers, assigned_tracks, num_curr, prev_num_dets, message)
